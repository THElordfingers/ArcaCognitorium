...
ENTITY_STORAGE = _ARCA_DIR / 'storage' / 'entities'

_CELESTIAL_YAML_TEMPLATE = """\
# Celestial configuration for {name}
# Generated by EntitexRefined on entity vault save.
# Edit to tune celestial influence weights and affinities.
circadian_weight: 0.40
celestial_weight: 0.40
solar_weight: 0.10
tidal_weight: 0.10
affinity_conditions: []
...
def _scaffold_celestial_yaml(entity):
    """Create a default celestial.yaml for a new entity if one doesn't exist."""
    name = entity.get('display_name', 'unnamed')
    slug = re.sub(r'\W+', '_', name.lower())
    entity_dir = ENTITY_STORAGE / slug
    yaml_path = entity_dir / 'celestial.yaml'
    if yaml_path.exists():
        return  # never overwrite existing config
    entity_dir.mkdir(parents=True, exist_ok=True)
    yaml_path.write_text(_CELESTIAL_YAML_TEMPLATE.format(name=name))
    log.info(f'Scaffolded celestial.yaml for {slug}')
...
class FornaxEntiumApp(QMainWindow):
    ...
    def _on_portrait_done(self, path):
        ...
        try:
            ...
            _scaffold_celestial_yaml(self._entity)
